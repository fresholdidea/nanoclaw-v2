...
describe('groups config update --provider-chain / --no-fallback', () => {
  ...
  it('sets provider_chain via --provider-chain (comma-separated)', async () => {
    const GID = 'ag-chain';
    createAgentGroup({ id: GID, name: 'chain', folder: 'chain', agent_provider: null, created_at: now() });
    ensureContainerConfig(GID);
    ...
    expect(resp.ok).toBe(true);
    const stored = getContainerConfig(GID)!.provider_chain;
  ...
  it('collapses chain to [primary] via --no-fallback', async () => {
    ...
    createAgentGroup({ id: GID, name: 'nofb', folder: 'nofb', agent_provider: null, created_at: now() });
    ensureContainerConfig(GID, 'codex');
  ...
  it('does NOT throw "Nothing to update" when only --provider-chain is given', async () => {
    ...
    createAgentGroup({ id: GID, name: 'co', folder: 'co', agent_provider: null, created_at: now() });
    ensureContainerConfig(GID);
  ...
  it('throws when --no-fallback and --provider-chain are both given', async () => {
    ...
    createAgentGroup({ id: GID, name: 'cf', folder: 'cf', agent_provider: null, created_at: now() });
    ensureContainerConfig(GID);
