  ...
  customOperations: {
    ...
    'config update': {
      ...
      description:
        ...
        'Use --id <group-id> and any of: --provider, --model, --effort, --image-tag, --assistant-name, --max-messages-per-prompt, --cli-scope, --provider-chain <p1,p2,...>, --no-fallback.',
      handler: async (args) => {
        const id = args.id as string;
        if (!id) throw new Error('--id is required');
        const row = getContainerConfig(id);
        ...
        if (Object.keys(updates).length === 0 && !hasProviderChain && !hasNoFallback) {
          throw new Error(
            'Nothing to update — provide at least one of: --provider, --model, --effort, --image-tag, --assistant-name, --max-messages-per-prompt, --cli-scope, --provider-chain, --no-fallback',
          );
        }
