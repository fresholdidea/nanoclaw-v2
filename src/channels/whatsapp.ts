      ...
      sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            // Echo filter. Two modes:
            //  - ASSISTANT_HAS_OWN_NUMBER=true: bot has its own number; every
            //    fromMe is the bot's own outbound coming back via sync. Skip.
            //  - ASSISTANT_HAS_OWN_NUMBER=false (paired-device): the bot IS
            //    the user's account on a companion device. fromMe is true for
            //    everything the user sends from any device on this account, so
            //    a blanket skip would drop the user's own messages. Distinguish
            //    bot replies by the "<ASSISTANT_NAME>: " prefix the deliver path
            //    adds (see formatOutbound below).
            if (ASSISTANT_HAS_OWN_NUMBER ? fromMe : isBotMessage) continue;
