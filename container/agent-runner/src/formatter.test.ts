...
describe('agent-to-agent source session attribution', () => {
  function insertA2aMessage(id: string, sourceSessionId: string | null, text: string) {
    getInboundDb()
      .prepare(
        `INSERT INTO messages_in (id, kind, timestamp, status, channel_type, platform_id, source_session_id, content, seq)
         VALUES (?, 'chat', ?, 'pending', 'agent', 'ag-peer-group', ?, ?, ?)`,
      )
      .run(id, new Date().toISOString(), sourceSessionId, JSON.stringify({ sender: 'cache-am', text }), nextSeq++);
  }
  ...
  it('never stamps session on non-agent channel messages', () => {
    getInboundDb()
      ...
      .run(new Date().toISOString(), JSON.stringify({ sender: 'Brad', text: 'hi' }), nextSeq++);
    const result = formatMessages(getPendingMessages());
    expect(result).not.toContain('session=');
  });
});
