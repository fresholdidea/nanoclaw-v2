...
- **Pinned ARGs** — `BUN_VERSION`, `MNEMON_VERSION`, `NOTEBOOKLM_CLI_VERSION`, `PNPM_VERSION`, `NPM_VERSION`, and `INSTALL_CJK_FONTS`. Bump deliberately in PRs. Global CLI versions (`@anthropic-ai/claude-code`, `agent-browser`, `vercel`) are pinned separately in `container/cli-tools.json`, not as ARGs.
- **`nlm` (NotebookLM CLI)** — `ARG NOTEBOOKLM_CLI_VERSION` installs `notebooklm-mcp-cli` via `uv tool install` against the system `python3` into world-readable `/opt/uv/tools` (+ `/usr/local/bin/nlm`), so the non-root `node` user can run it. Adds ~73MB (uv-managed venv + Python deps; **no** Playwright/browser — NotebookLM queries run off a mounted cookie file over HTTP, and interactive `nlm login` happens on the host, not in-container). Credentials arrive via a per-group `additionalMounts` bind of `~/.notebooklm-mcp-cli` → `/workspace/extra/notebooklm`, which `ENV NOTEBOOKLM_MCP_CLI_PATH` points the CLI at — the same shape as `mnemon`/`MNEMON_DATA_DIR`.
