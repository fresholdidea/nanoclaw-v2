...
**Thread-scoped wirings**: `messaging_group_agents.thread_filter` (migration 024) confines a wiring to one or more comma-separated effective thread ids (e.g. a Telegram forum topic `telegram:-100123:4`). At fanout a scoped wiring engages only when its effective thread id equals the filter; when any scoped wiring matches, every unscoped wiring on the same messaging group stands down for that message, so an orchestrator wired without a filter answers every other topic without double-answering beside the specialist. Scoped wirings never accumulate outside their thread. `ncl wirings create|update --thread-filter <topic-number|full-thread-id>`; requires the wiring's thread policy to resolve on (same check as `session_mode 'per-thread'`).

**Thread-pinned destinations**: `agent_destinations.thread_id` (migration 025) pins a channel destination to one thread/topic. The projection into each session's `inbound.db` carries it, and every send through that name (`send_message`, `send_file`, `<message to>` blocks) stamps it as the outbound `thread_id`, overriding the session-derived thread. NULL keeps the old behaviour. `ncl destinations add … --thread <topic-number|full-thread-id>`; re-adding an existing local name replaces its target and thread. This is how a topic-scoped agent's scheduled-task output lands in its own topic of an orchestrator room.
